...
const Appointment = () => {
  ...
  const bookAppointment = async () => {
    if(!token){
      toast.warn('Login to book appointemt')
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
          ...
          <p className="text-gray-500 font-medium mt-4">
            Appointment fee:<span className="text-gray-700">{currencySymbol}{docInfo.fees}</span> </p>
        </div>
      </div>
      ...
      <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
        ...
        {doctorSlots.length && doctorSlots[slotIndex].map((item, index)=>(
            ...
            className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white': 'text-gray-500 border border-gray-300 '} `}
              key={index}>
              {item.time.toLowerCase()}
            </p>
        ))}
        </div>
        <button onClick={bookAppointment} className="bg-primary text-white text-sm font-light px-14 py-3 rounded-full mt-5">
          Book an appoitment
        </button>
