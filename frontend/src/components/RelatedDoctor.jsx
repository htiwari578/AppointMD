        ...
        <div className="w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 sm:px-0">
            {
                relatedDoc.slice(0,5).map((item , index)=>(
                    ...
                    hover:translate-y-[-10px] transition-all duration-500" key ={index}> 
                        ...
                        <div className="p-4">
                        <div className={`flex items-center gap-2 text-sm text-center ${item.available ? 'text-green-700' : 'text-gray-500'} `}>
                            <p className={`w-2 h-2 ${item.available ? 'bg-green-700 ' : 'bg-gray-500'} rounded-full`}></p><p>{item.available ? 'Available' : 'Not Available'}</p>
                        </div>
